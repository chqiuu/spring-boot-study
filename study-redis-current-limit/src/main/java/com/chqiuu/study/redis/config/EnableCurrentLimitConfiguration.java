...
@Slf4j
@Configuration
@ComponentScan(basePackages = "com.chqiuu.study.redis")
public class EnableCurrentLimitConfiguration {
    ...
    public DefaultRedisScript<Long> getTokenBucketRedisScript() {
        ...
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("script/redis-token-bucket-limiter.lua")));
        return redisScript;
    }
    ...
    public DefaultRedisScript<Long> getCounterRedisScript() {
        DefaultRedisScript<Long> redisScript = new DefaultRedisScript<>();
        redisScript.setResultType(Long.class);
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("script/redis-counter-limiter.lua")));
        return redisScript;
    }
}
